"""
MS/MS mirror plot: Dopamine (CCMSLIB00005733696, top) vs Octopamine (CCMSLIB00006683690, bottom).

Peaks fetched live from the GNPS2 metabolomics-usi JSON endpoint (fragment_mz_tolerance=0.1).
Shared/annotated fragments (m/z 91.05, 107.05, 119.05 — common catecholamine ring/side-chain
fragments) are highlighted; all other peaks are drawn in the neutral stem color.
"""
...
OUT_STATIC = os.path.join(BASE, "mirror_plot_dopamine_octopamine.svg")

USI_TOP    = "mzspec:GNPS:GNPS-LIBRARY:accession:CCMSLIB00005733696"  # Dopamine
USI_BOTTOM = "mzspec:GNPS:GNPS-LIBRARY:accession:CCMSLIB00006683690"  # Octopamine

TOP_NAME    = "Dopamine"
BOTTOM_NAME = "Octopamine"
TOP_SMILES    = "NCCc1ccc(O)c(O)c1"
BOTTOM_SMILES = "NCC(O)c1ccc(O)cc1"

MZ_MIN, MZ_MAX = 40.0, 160.0
FRAGMENT_TOL = 0.1
ANNOTATE_PEAKS = [91.0542, 107.0491, 119.0491]  # shared ring/side-chain fragments
